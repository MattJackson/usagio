...
> **Renamed from `claude-usage`.** v0.4.0 renames the project to `usagio`; upgrading
> Homebrew users get the new binary transparently, and on first run the config
> directory (`~/.config/claude-usage/` → `~/.config/usagio/`) and launchd label
> migrate themselves. The macOS Keychain service string is intentionally frozen
> at `"claude-usage"` so existing tokens survive without a re-login.
...
`usagio` solves all three. Accounts are identified by their **email** — there
...
cd usagio
cargo build --release
cp target/release/usagio /usr/local/bin/
```

## Onboarding accounts

You log into each account once, the normal way, and `usagio` **captures** that
...
`~/.config/usagio/state.json`), then log in as the next one and capture that.
...
usagio capture          #  ->  Captured you@work.com

# 2. Log in as your second account, then capture it.
claude              # /login as account B
usagio capture          #  ->  Captured you@personal.com
...
`usagio menubar` is the primary interface: a status-bar item showing your
...
The menu bar also runs the auto-swap daemon in the same process. `usagio
install` registers a `launchd` agent so it starts at every login (and runs
Dock-less — menu bar only). `usagio uninstall` removes it.
...
```
usagio                     Show usage for every account (default)
usagio capture             Capture the account you're currently logged into
usagio switch [acct]       Make <acct> the active login (no launch)
...
usagio install|uninstall   Run / stop the menu-bar app at every login
...
```sh
usagio start               # auto-pick the best account and open claude
usagio continue dev        # prefix-match dev@…, switch, resume last conversation
TOKEN=$(usagio token dev)  # a fresh access token for scripts
```
...
- Tokens live locally in `~/.config/usagio/state.json` (owner-only, `0600`)
...
- If `ANTHROPIC_API_KEY` is set in your environment, Claude Code uses that and
  ...
  unset. `usagio` manages subscription (OAuth) logins, not API keys.
