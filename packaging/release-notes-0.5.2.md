## usagio v0.5.2 — robustness + install-UX round
...
gets a hang-proof timeout, "install means install" everywhere, and a batch
of concurrency/state and menu-UX fixes from the latest audit pass. No
schema or CLI-flag changes — safe to `brew upgrade` / re-run the Windows
installer in place.
...
- **Menu UX: account blocks separated by HR.** Accounts render as
  ...
  the previous per-account single-line rows. Flatter Capture and Settings
  submenus; consolidated auto-swap under `Settings ▸ Auto-swap ▸`.
- **Weekly-aware adaptive cadence.** Poll cadence now tightens when
  either session % _or_ weekly % approaches the trigger threshold —
  previously session-only.
- **Stale-after-reset display.** When a weekly window has just reset but
  the cached usage predates the reset, the menu now renders `-% / -%`
  (with a boundary wake) instead of showing stale `0% / 100%`.
